...
RUN apt update && \
    ...
    echo "#!/bin/bash" >> /home/shade/beit_seg/start.sh && \
    echo "source /opt/shade/setup.sh" >> /home/shade/beit_seg/start.sh && \
    echo "source /opt/ros/${ROS_VERSION}/setup.sh" >> /home/shade/beit_seg/start.sh && \
    echo "source ./install/setup.sh" >> ./start.sh && \
    echo "ros2 run beit_seg beit_seg" >> /home/shade/beit_seg/start.sh && \
    chmod +x ./start.sh
